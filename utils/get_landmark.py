...
import torch
import torchvision
from utils.detect_face import SCRFD
# from models.pfld_lite import PFLDInference
# from models.pfld import PFLDInference
from utils.pfld_mobileone import PFLD_GhostOne as PFLDInference
...
class Landmark:
    def __init__(self):
        ...
        with open('pretrained_models/landmark/mean_face.txt', 'r') as f_mean_face:
            mean_face = f_mean_face.read()
        self.mean_face = np.asarray(mean_face.split(' '), dtype=np.float32)
        self.det_net = SCRFD('pretrained_models/landmark/scrfd_2.5g_kps.onnx', confThreshold=0.1, nmsThreshold=0.5)

        checkpoint = torch.load('pretrained_models/landmark/checkpoint_epoch_335.pth.tar')
